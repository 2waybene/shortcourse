...
GSEA.EnrichmentScore <- function(gene.list, gene.set, weighted.score.type = 1, correl.vector = NULL) {  
  ...
  max.ES <- max(RES)
  min.ES <- min(RES)
  if (max.ES > (- min.ES)) { ##  JYL trouble shoot 
    #      ES <- max.ES
...
GSEA.EnrichmentScore2 <- function(gene.list, gene.set, weighted.score.type = 1, correl.vector = NULL) {  
  ...
  ES <- signif(ifelse(max.ES > (- min.ES), max.ES, min.ES), digits=5) ## JYL added prenthesiss
...
GSEA.Gct2Frame2Mod <- function(filename = "NULL") 
  ##  JYL for debugging
{
  f <- read.table (filename, sep="\t", header = TRUE)
  row.nam<- as.character(f[,1])
  col.nam<- colnames(f)[-1]
  ds <- data.frame(f[,-1])
  colnames(ds) <- col.nam
  row.names(ds) <- row.nam
  return(ds)
...
#  class.list <- unlist(strsplit(cls.cont[[3]], " ")) #JYL -modified 7/20/16
...
GSEA <- function(
  ...
  if (is.data.frame(input.ds)) {
    ...
    if (regexpr(pattern=".gct", input.ds) == -1) {
     ...
     # dataset <- GSEA.Gct2Frame2(filename = input.ds)     ## JYL trouble shoot 01/24/2020
      dataset <- GSEA.Gct2Frame2Mod(filename = input.ds)  ## JYL trouble shoot 01/24/2020
    }
  }
